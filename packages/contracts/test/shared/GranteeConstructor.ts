...
export const granteeConstructorTests = async (
  ...
  describe('GranteeConstructor - Constructor Tests', async () => {
    ...
    it('should record correct grantee references', async () => {
      expect(await _contract.getGranteeReferenceLength()).to.eq(
        _grantees.length,
      );

      for (let i = 0; i < _grantees.length; i += 1) {
        expect(await _contract.getGranteeReference(i)).to.eq(
          await _grantees[i].getAddress(),
        );
      }
    });

    it('should record correct grantee amounts', async () => {
      const granteeTargetFunding = await getGranteesTargetFunding(
        _contract,
        _grantees,
      );
