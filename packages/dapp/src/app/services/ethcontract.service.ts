...
export class EthContractService {
  ...
  createGrant(data) {
    return new Promise(async (resolve, reject) => {
      ...
      if (data.currency === 'ETH') {
        ...
        data.targetFunding = ethers.utils
          .parseEther(data.targetFunding.toString())
          .toString();
      }
      ...
      const contract = new ethers.Contract(
        environment.ethersConfig.factoryContract,
        GrantFactory.abi,
        provider,
      );
    ...
    return new Promise(async resolve => {
      try {
        const provider = ethers.getDefaultProvider(
          environment.ethersConfig.network,
        );
        const contract = new ethers.Contract(
          contractAddress,
          ManagedCappedGrantAbi.abi,
          provider,
        );
  ...
  approvePayout(contractAddress, granteePublicKey, amount) {
    return new Promise(async (resolve, reject) => {
      ...
      const valuetkn = ethers.BigNumber.from(amount);
      contractWithSigner
        .approvePayout(valuetkn, granteePublicKey, {
          gasLimit: environment.ethersConfig.gasLimit,
        })
        .then(
          response => {
  ...
  approveRefund(contractAddress, userPublicKey, amount) {
    return new Promise(async (resolve, reject) => {
      ...
      const contractWithSigner = contract.connect(signer);
      const valuetkn = ethers.BigNumber.from(amount);
      const response = await contractWithSigner.approveRefund(
        valuetkn,
        userPublicKey,
        { gasLimit: environment.ethersConfig.gasLimit },
      );
  ...
  cancelGrant(contractAddress) {
    return new Promise(async (resolve, reject) => {
      ...
      const contractWithSigner = contract.connect(signer);
      contractWithSigner
        .cancelGrant({ gasLimit: environment.ethersConfig.gasLimit })
        .then(
          async response => {
            resolve({
              status: 'success',
  ...
  parseTransaction(input) {
    ...
    const data = {
      ...
      fundingDeadline: moment(
        +parseData.args[5].toString(16).toUpperCase(),
      ).format(),
      contractExpiration: moment(
        +parseData.args[6].toString(16).toUpperCase(),
      ).format(),
